...
def optimize(policy_net, target_net, memory):
    ...
    for state, action, reward, state_next, terminal, run in batch:
        ...
        if not terminal:
            ...
            # for terminal actions Q = reward
            expected_state_action_values = policy_net(torch.FloatTensor(state))
            # Detach values from gradients to prevent gradient update
            expected_state_action_values = expected_state_action_values.detach()
            # Set Q for all actions to reward
            expected_state_action_values[0][action] = reward
...
def order_policy():
    ...
    while continue_learning:
        ...
        while True:
            ...
            next_pending_parts = state_next[1]
            next_last_stock_count = state_next[2]
            ...
            pending_parts = next_pending_parts
