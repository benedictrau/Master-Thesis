...
def get_action(class_probability, prediction, string, mod, neurons_per_layer):
    ...
    model = NN(features, action_space, neurons_per_layer=neurons_per_layer)
...
def get_parameters(parameter):
    ...
    default_lr = 0.00025
    default_gamma = 0.99
    default_memory_size = 15000
    default_batch_size = 32
    default_replay_start_size = 2000
    default_synch_target_steps = 200
    default_exp_max = 1
    default_exp_min = 0.1
    ...
    gamma_list = [0.89, 0.79]
    memory_size_list = [7500, 30000]
    batch_size_list = [16, 64]
    replay_start_size_list = [1000, 4000]
    synch_target_steps_list = [100, 400]
    exp_max_list = [1, 1]
    exp_min_list = [0.05, 0.2]
    exp_decay_list = [0.89, 0.79]
    neurons_per_layer_list = [16, 64]
    ...
    if parameter == "lr":
        ...
        list = neurons_per_layer_list
    elif parameter == "dropout_rate":
        list = dropout_rate_list
    ...
    return lr, gamma, memory_size, batch_size, replay_start_size, synch_target_steps, \
           exp_max, exp_min, exp_decay, neurons_per_layer, dropout_rate, list


def get_default():

    lr, gamma, memory_size, batch_size, replay_start_size, synch_target_steps, \
    exp_max, exp_min, exp_decay, neurons_per_layer, dropout_rate, list = get_parameters(parameter="lr")
    ...
    order_policy(learning_rate=lr, gamma=gamma, memory_size=memory_size, batch_size=batch_size,
                 ...
                 train_epochs=train_epochs, train_sim_dur=train_sim_dur, neurons_per_layer=neurons_per_layer,
                 dropout_rate = dropout_rate)

    default_results = order_policy_eval(default_string, test_sim_dur=test_sim_dur, test_epochs=test_epochs, mod="MLS",
...
def evaluation(mod, varied_parameter):
    ...
    lr, gamma, memory_size, batch_size, replay_start_size, synch_target_steps, \
    exp_max, exp_min, exp_decay, neurons_per_layer, dropout_rate, list = get_parameters(parameter=varied_parameter)
    run = 0
    ...
    for i in list:
        ...
        if varied_parameter == "lr":
            ...
            exp_decay = i
        elif varied_parameter == "neurons_per_layer":
            neurons_per_layer = i
        elif varied_parameter == "dropout_rate":
            dropout_rate = i
        ...
        order_policy(learning_rate = lr, gamma = gamma, train_sim_dur=train_sim_dur,
                     ...
                     synch_target_steps=synch_target_steps, memory_size=memory_size, dropout_rate=dropout_rate,
                     neurons_per_layer=neurons_per_layer)
        ...
        total_results_dr.append(dropout_rate)
        total_reults_neurons.append(neurons_per_layer)
    ...
    total_results['replay_start_size'] = total_results_replay_start_size
    total_results['synch_target_steps'] = total_results_synch_target_steps
    total_results['exp_max'] = total_results_exp_max
    total_results['exp_min'] = total_results_exp_min
    total_results['exp_decay'] = total_results_exp_decay
    total_results['dropout_rate'] = total_reults_neurons
...
#varied_parameters = ['neurons_per_layer', 'dropout_rate']

varied_parameters = ['lr', 'gamma', 'memory_size', 'batch_size', 'replay_start_size',
                     'synch_target_steps', 'exp_min', 'exp_decay', 'neurons_per_layer',
                     'dropout_rate']

get_default()
#for param in varied_parameters:
#    run = evaluation(mod="MLS", varied_parameter=param)
